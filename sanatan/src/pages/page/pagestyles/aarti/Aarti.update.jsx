...
function Aarticontentupdate() {
	...
	async function fetchdata() {
		try {
			const response = await axios.get(
				`${process.env.REACT_APP_SERVER}page/aarti/${id}`
			);
	...
	const Savedata = async (e) => {
		e.preventDefault();
		try {
			const Api = `${process.env.REACT_APP_SERVER}page/aarti/${id}`;
			await axios
				.put(Api, Page)
				.then((res) => {
					const notify = () =>
...
export default Aarticontentupdate;
