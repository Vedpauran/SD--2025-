...
function BlogContentUpdate() {
	...
	function fetchdata() {
		try {
			axios
				.get(`${process.env.REACT_APP_SERVER}blogs/content/${id}`)
				.then((res) => {
	...
	const Savedata = async (e) => {
		e.preventDefault();
		try {
			const Api = `${process.env.REACT_APP_SERVER}blogs/edit/${id}`;
			await axios
				.put(Api, Page)
				.then((res) => {
					const notify = () =>
...
export default BlogContentUpdate;
