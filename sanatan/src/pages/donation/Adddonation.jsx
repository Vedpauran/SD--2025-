...
const Api = `${process.env.REACT_APP_SERVER}donation/add`;
...
function Adddonation() {
	...
	const [AddLanguage, Languagearray] = useState([]);
	...
	const handleLanguages = (value) => {
		AddLanguage.push(value);
	};
	function removelanguage(value) {
		const index = AddLanguage.indexOf(value); // Check if the value already exists in the AddLanguageay

		if (index !== -1) {
			// If the value exists, remove it
			AddLanguage.splice(index, 1);
		} else {
			// If the value doesn't exist, add it
			AddLanguage.push(value);
		}

		setPage({ ...Page, Languages: AddLanguage });
	}
	...
	async function pageRedirector(language) {
		if (Page.category === "" || Page.title === "") {
			const notify = () =>
				toast.error("title and categories required", {
					position: "bottom-right",
					autoClose: 5000,
					hideProgressBar: false,
					closeOnClick: true,
			...
			try {
				await axios
					...
					.then((res) => {
						console.log(res);
						navigate(
							`/donations/add/content/${res.data._id}/${language}`
						);
					})
	...
	return (
		<div>
			<h1>Donation / Add Donation (Content)</h1>
			<div className="Card">
			...
			<div className="Card tablecard">
				...
				<FileInputComponent
					title="Upload Image or add link"
					links={Page.banner}
					onDelete={() => setPage({ ...Page, banner: "" })}
					onAdd={(image) => setPage({ ...Page, banner: image })}
					type={"single"}
				/>
			</div>
			<div className="Card tablecard">
