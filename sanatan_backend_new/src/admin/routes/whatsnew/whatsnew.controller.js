...
const Whatsnew = require("../../models/whatsnew/whatsnew");
...
exports.getWhatsnewByLanguage = async (req, res) => {
	try {
		const Whatsnews = await Whatsnew.findOne({
			Page: req.params.id,
...
exports.getTrashWhatsnews = async (req, res) => {
	try {
		const whatsnew = await Whatsnew.find({ isTrash: true });
		if (!whatsnew) {
			return res.status(404).json({ message: "Whatsnew not found" });
		}
		res.json(whatsnew);
