...
// Find blogs with totals and filtering
      ...
      const pipeline = [
        {
          $facet: {
            ...
            // Filter activeBlogs based on 'type' query, excluding trash unless explicitly requested
            activeBlogs: [
              {
                $match: type ? { status: type } : { status: { $ne: "trash" } }, // default excludes 'trash'
              },
              {
                $project: {
                  _id: 1,
...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    if (blog.status === "trash") {
      ...
      return res.status(200).json({ msg: "Blog permanently deleted" });
    } else {
      await Blog.findByIdAndUpdate(_id, {
        status: "trash",
        deletedAt: new Date(),
      });
      return res.status(200).json({ msg: "Blog moved to trash successfully" });
    }
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
