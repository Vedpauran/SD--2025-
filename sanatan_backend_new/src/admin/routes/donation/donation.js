...
router.delete("/:id", async (req, res) => {
  ...
  try {
    ...
    if (donation.status === "trash") {
      await Donation.findByIdAndDelete(_id);
      return res.status(200).json({ msg: "Donation permanently deleted" });
    } else {
      await Donation.findByIdAndUpdate(_id, {
        status: "trash",
        deletedAt: new Date(),
      });
      return res
        .status(200)
        .json({ msg: "Donation moved to trash successfully" });
    }
  } catch (error) {
    res.status(500).json({ message: error.message });
  }
