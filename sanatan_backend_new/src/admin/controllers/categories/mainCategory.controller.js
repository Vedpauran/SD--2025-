          ...
          totals: [
            {
              $group: {
                ...
                trash: {
                  $sum: {
                    $cond: [{ $eq: ["$Status", "trash"] }, 1, 0],
                  },
                },
                all: {
                  $sum: {
                    $cond: [{ $ne: ["$Status", "trash"] }, 1, 0],
                  },
                },
              },
            },
  ...
  try {
    ...
    if (!mongoose.Types.ObjectId.isValid(id)) {
      return res.status(400).json({
        success: false,
        message:
          "Invalid category ID format. Must be a 24-character hex string.",
      });
    }
    ...
    console.error("🔥 Error processing category deletion:", err.message);
