...
exports.getExtrapageById = async (req, res) => {
	try {
		...
		if (!extrapage) {
			return res.status(404).json({ message: "Extrapage not found" });
		}
		res.json(extrapage);
	} catch (err) {
		res.status(500).json({ message: err.message });
...
exports.deleteExtrapage = async (req, res) => {
	try {
		...
		res.json({ message: "Extrapage deleted successfully" });
	} catch (err) {
		res.status(500).json({ message: err.message });
	}
};
...
exports.getTrashExtrapages = async (req, res) => {
	try {
		const extrapage = await Extrapage.find({ isTrash: true });
		if (!extrapage) {
			return res.status(404).json({ message: "Extrapage not found" });
		}
		res.json(extrapage);
